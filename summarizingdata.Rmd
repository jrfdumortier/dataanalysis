...
2. ***Household Pulse Survey*** (\*\*\*): The U.S. Census Bureau has created a weekly survey to *"quickly and efficiently deploy data collected on how people's lives have been impacted by the coronavirus pandemic."* You find three tools to explore the data on the survey's [webpage](https://www.census.gov/data/experimental-data-products/household-pulse-survey.html): (1) Interactive Tool, (2) Data Tables, and (3) Public Use Files (PUF). The PUF contains original data. For this exercise, download the PUF of the most recent week available in csv-format, which is denoted "HPS Week X PUF CSV". Extract the zip-file and keep the two necessary files for this exercise: `pulse2020_puf_X.csv` and `pulse2020_data.dictonary_CSV_X.csv`. Note that the file names may be slightly different depending on the year. Column $Variable$ in `pulse2020_data.dictonary_CSV_X.csv` describes the columns in `pulse2020_puf_X.csv`. 

     a. Pick two variables which are of interest to you, e.g., "Educational attainment" and "Frequency of worry over previous 7 days," and construct a cross-table using the package [gmodels](https://cran.r-project.org/web/packages/gmodels/index.html). 
     b. What is your interpretation of the results obtained in part (a)? Is it what you expected? What do you conclude from a policy perspective? 
     
     Here is a hing how the R-code would look like this for the relationship between worrying and education:

```{r SUMhouseholdpulsesurvey,results=FALSE,eval=FALSE}
