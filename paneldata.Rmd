...
```{r}
bhat = lm(formula=lwage~y85+educ+y85*educ+exper+expersq+union+female+y85fem,data=cps7885)
summary(bhat)
```
...
Include $age$ and $age^2$ in the above equation to take advantage of the information provided in the data. Other variables such as $cbd$, $rooms$, $area$, $land$, and $baths$ can be added as well. In general, the results show that homes have lost 9.3% in values when including additional independent variables and using the natural logarithm of price. 
...
The companies of interest for this chapter are GM (firm 1), U.S. Steel (firm 2), GE (firm 3), and Westinghouse (firm 8).
...
```{r}
summary(plm(inv~value+capital,data=grunfeld,model="pooling"))
```

\begin{frame}{Fixed Effects Model}{Theoretical Concepts I}
    Fixed effects model or Least-Squares Dummy Variable (LSDV) regression
        \begin{itemize}
            \item Constant slope coefficients but varying intercept over $i$
        \end{itemize}
    Regression equation:
        \begin{gather*}
...
\end{frame}
%========================================================================================================================================
\begin{frame}{Fixed Effects Model}{Theoretical Concepts II}
    Fixed effects model
        \begin{itemize}
            \item Intercept $\beta_{0i}$ is firm specific.
            \item For an individual, this could be education and/or ability, possibly correlated with independent variables
            \item Intercept is time-invariant.
            \item Slope coefficients do not vary across individuals (firms) or time
        \end{itemize}
    Implementation in R
        \begin{itemize}
            \item Using the \texttt{plm} function and specifying the model as \texttt{``within''}
            \item Name the output \texttt{``grunwald.fixed''}
        \end{itemize}
\end{frame}

```{r}
summary(plm(inv~value+capital,data=grunfeld,model="within"))
```
