...
```{r}
df        = subset(gss,vote20 %in% c(1,2) & degree %in% c(0:4),select=c("vote20","degree"))
CrossTable(df$degree,df$vote20,prop.r = FALSE,prop.c = FALSE,prop.chisq = FALSE,prop.t=FALSE)
```
...
```{r}
chisq.test(df$degree,df$vote20)
```
...
votegun   = subset(gss,
                   vote20 %in% c(1,2) &
                   owngun %in% c(1,2),
                   select = c("vote20","owngun"))
votegun$vote20 = ifelse(votegun$vote20==1,1,0)
votegun$owngun = ifelse(votegun$owngun==1,1,0)
t.test(votegun$vote20~votegun$owngun)
```
```{r}
chisq.test(votegun$vote20,votegun$owngun)
```
...
```{r}
CrossTable(votegun$vote20,votegun$owngun,prop.r = FALSE,prop.c = FALSE,prop.chisq = FALSE,prop.t=FALSE)
```
